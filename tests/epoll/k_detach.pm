  ...
  ( inline_states =>
    { _start => sub {
      ...
      run_tests => sub {
        ...
        ok_if( 1, $test_trace eq '(c 1 lose a1_1)(p a1_1 1 kernel)' );

        $test_trace = '';
        $kernel->call( a2_1 => 'detach_self' );
        ok_if( 2, $test_trace eq '(c 2 lose a2_1)(p a2_1 2 kernel)' );

        $test_trace = '';
        $kernel->call( a3_1 => 'detach_self' );
        ok_if( 3, $test_trace eq '(c 3 lose a3_1)(p a3_1 3 kernel)' );

        $test_trace = '';
        $kernel->call( a1 => detach_child => 'a1_2' );
        ok_if( 4, $test_trace eq '(c 1 lose a1_2)(p a1_2 1 kernel)' );

        $test_trace = '';
        $kernel->call( a2 => detach_child => 'a2_2' );
        ok_if( 5, $test_trace eq '(c 2 lose a2_2)(p a2_2 2 kernel)' );
        ...
        ok_if( 7, $test_trace eq '(c main lose 1)(p 1 main kernel)' );

        $test_trace = '';
        $kernel->call( main => detach_child => 'a2' );
        ok_if( 8, $test_trace eq '(c main lose 2)(p 2 main kernel)' );
      },
      _parent => sub {
